...
On completion, the plugin shows a visible TUI toast and inserts the capped final result as synthetic context that stays hidden in the transcript but is available to the main agent on its next turn. The notification does not create a user-shaped row or start an assistant turn itself.
